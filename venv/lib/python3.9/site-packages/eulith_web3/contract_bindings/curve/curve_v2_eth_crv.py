...
class CurveV2EthCrv:
    ...
    def add_liquidity(self, amounts: List[int], min_mint_amount: int, use_eth: bool, override_tx_parameters: Optional[TxParams] = None) -> TxParams:
        if not self.address:
            raise ContractAddressNotSet("you must either deploy or initialize the contract with an address")
        c = self.w3.eth.contract(address=self.address, abi=self.abi)
                    
        return c.functions.add_liquidity(amounts, min_mint_amount, use_eth).build_transaction(override_tx_parameters)
    ...
    def exchange_underlying(self, i: int, j: int, dx: int, min_dy: int, override_tx_parameters: Optional[TxParams] = None) -> TxParams:
        if not self.address:
            raise ContractAddressNotSet("you must either deploy or initialize the contract with an address")
        c = self.w3.eth.contract(address=self.address, abi=self.abi)
                    
        return c.functions.exchange_underlying(i, j, dx, min_dy).build_transaction(override_tx_parameters)
    ...
    def remove_liquidity(self, _amount: int, min_amounts: List[int], use_eth: bool, override_tx_parameters: Optional[TxParams] = None) -> TxParams:
        if not self.address:
            raise ContractAddressNotSet("you must either deploy or initialize the contract with an address")
        c = self.w3.eth.contract(address=self.address, abi=self.abi)
                    
        return c.functions.remove_liquidity(_amount, min_amounts, use_eth).build_transaction(override_tx_parameters)

    def remove_liquidity_one_coin(self, token_amount: int, i: int, min_amount: int, use_eth: bool, override_tx_parameters: Optional[TxParams] = None) -> TxParams:
        if not self.address:
            raise ContractAddressNotSet("you must either deploy or initialize the contract with an address")
        c = self.w3.eth.contract(address=self.address, abi=self.abi)
                    
        return c.functions.remove_liquidity_one_coin(token_amount, i, min_amount, use_eth).build_transaction(override_tx_parameters)
